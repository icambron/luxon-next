...
const getOpts = (
  locale: FormatFirstArg<RelativeFormatOpts>,
  format: FormatSecondArg<RelativeFormatOpts>
): RelativeFormatOpts => {
...
const findBiggestUnit = (
  start: DateTime,
  end: DateTime,
  threshold: number,
  precision: number,
  units: RelativeUnit[]
): [Duration, RelativeUnit] => {
...
export const toRelativeHuman = (
  ...
  if (highOrderUnit === "days") {
    if (Math.abs(durGet(highOrderDur, "days")) < 0.5) {
      // a magic number. Switch to hours below this
