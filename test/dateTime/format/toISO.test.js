import { fromGregorian, fromMillis, setTime, toISO, toISOAdvanced, toFixedOffset, setZone } from "../../../src/luxon";
...
test("toISOAdvanced() shows 'Z' for UTC", () => {
  expect(toISOAdvanced(dt)).toBe("1982-05-25T09:23:54.123Z");
});

test("toISOAdvanced() shows the offset, unless explicitely asked", () => {
...
test("toISOAdvanced() supports the 'basic' format", () => {
  expect(toISOAdvanced(dt, { format: "basic" })).toBe("19820525T092354.123Z");
});

test("toISOAdvanced({ elideZeroMilliseconds }) suppresses milliseconds", () => {
...
test("toISOAdvanced({ elideZeroSeconds }) suppresses seconds", () => {
...
test("toISOAdvanced() rounds fractional timezone minute offsets", () => {
