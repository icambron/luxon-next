...
import { values, months, days, milliseconds } from "../../../src/duration/core";
...
const diffObjs = (o1, o2, units) => diffFromObjs(o1, o2, units) |> values;
...
test("diff() makes simple diffs", () => {
  ...
  expect( diffObjs({ year: 2016, month: 6, day: 28 }, { year: 2016, month: 5, day: 28 }, "months")).toEqual({ months: 1 });
...
test("diff() sets all its units to 0 if the duration is empty", () => {
  ...
  expect(diff(t, t) |> values).toEqual({ milliseconds: 0 });
  expect(diff(t, t, "hours") |> values).toEqual({ hours: 0 });
  expect(diff(t, t, "days") |> values).toEqual({ days: 0 });
});
